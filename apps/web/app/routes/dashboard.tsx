...
export async function loader({ context }: Route.LoaderArgs) {
  ...
  const friendActivity = await trpc.gathering.friendActivity
  ...
  return {
    ...
    gatherings,
    joinedGatherings,
    friendActivityCount: friendActivity.total,
  }
}
                    ...
                    {gathering.status === 'active' ? (
                      ...
                      <Form
                        ...
                        onSubmit={(e) => {
                          if (
                            !confirm(
                              `Delete "${gathering.title}"? This cannot be undone.`,
                            )
                          ) {
                            e.preventDefault()
                          }
        ...
        <div className="animate-fade-in-up animation-delay-200 space-y-4 mt-10">
          ...
          {joinedGatherings.length === 0 ? (
            <div className="rounded-lg border border-border bg-card/60 p-10 text-center backdrop-blur-sm">
              <p className="text-sm text-muted-foreground">
                You haven&apos;t joined any gatherings yet.
              </p>
              <Button className="mt-4" asChild>
                  ...
                  <div className="flex items-center justify-between gap-4">
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-1 flex items-center gap-3 text-xs text-muted-foreground">
                        ...
                        <Badge
                          variant={
                            gathering.participantStatus === 'joined'
                              ? 'default'
                              : 'secondary'
                          }
                          className="text-[10px]"
