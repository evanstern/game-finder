...
export async function loader({ request, context }: Route.LoaderArgs) {
  ...
  const gameTypes = typesParam
    ? (typesParam.split(',').filter(Boolean) as GameType[])
    : undefined
  const sortBy = (url.searchParams.get('sort') ?? 'distance') as
    | 'distance'
    | 'next_session'
        ...
        <form onSubmit={handleSearch} className="mb-8">
          <div className="flex flex-wrap items-end gap-3">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <Select
                value={String(radiusInput)}
                onValueChange={(v) => setRadiusInput(Number(v))}
              >
              ...
              <div className="sticky top-8 space-y-6">
                  ...
                  <div className="flex flex-col gap-1">
                    {(
                      ...
                      <Button
                        key={type}
                        variant={
                          urlTypes?.includes(type) ? 'secondary' : 'ghost'
                        }
                        size="sm"
                        className={
                ...
                <div>
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <Button
                      variant={
                        urlSort === 'next_session' ? 'secondary' : 'ghost'
                      }
                      size="sm"
                      className={
                        urlSort === 'next_session'
                          ? 'justify-start text-primary'
                          : 'justify-start text-muted-foreground'
                      }
                              ...
                              {gathering.games.length > 0 && (
                                <div className="mt-2 flex flex-wrap gap-1">
                                  {[
                                    ...new Set(
                                      gathering.games.map(
                                        (game) => game.type as GameType,
                                      ),
                                    ),
                                  ].map((type) => (
                                    <GameTypeBadge key={type} type={type} />
                                  ))}
