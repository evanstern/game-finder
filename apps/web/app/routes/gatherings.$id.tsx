...
export async function loader({ request, params, context }: Route.LoaderArgs) {
  ...
  const [user, gathering, participants] = await Promise.all([
    ...
    trpc.gathering.listParticipants
      .query({ gatheringId: params.id })
      .catch(() => []),
  ])
      ...
      <div className="relative z-10 mx-auto max-w-4xl px-6 py-10 space-y-8">
        <div className="animate-fade-in-up flex items-start justify-between gap-4">
            ...
            <div className="text-sm text-muted-foreground flex items-center gap-2 flex-wrap">
              <span>
                ...
                <span className="font-medium text-primary">
                  {gathering.host.displayName}
                </span>
              </span>
              {user &&
                !isOwner &&
                (() => {
                  const hostIsFriend = friends.some(
                    (f) => f.friendId === gathering.hostId,
                  )
                  const hostIsPending =
                    outgoingRequests.some(
                      (r) => r.addresseeId === gathering.hostId,
                    ) ||
                    incomingRequests.some(
                      (r) => r.requesterId === gathering.hostId,
                    )
                  if (hostIsFriend)
                    return (
                      <span className="text-[10px] text-primary">Friend</span>
                    )
                  if (hostIsPending)
                    return (
          ...
          <div className="flex items-center gap-2">
            <Badge
              variant={gathering.status === 'active' ? 'default' : 'secondary'}
            >
                ...
                {gathering.status === 'active' && (
                  <Form
                    ...
                    onSubmit={(e) => {
                      if (
                        !window.confirm(
                          'Are you sure you want to close this gathering? This cannot be undone.',
                        )
                      ) {
                        e.preventDefault()
                      }
        ...
        <div className="animate-fade-in-up animation-delay-100 rounded-lg border border-border bg-card/60 p-6 backdrop-blur-sm space-y-4">
          <div className="grid grid-cols-2 gap-5 text-sm">
            <div>
              <p className="font-semibold text-[11px] tracking-[0.15em] text-primary uppercase mb-1.5">
                Schedule
              </p>
              <ScheduleLabel
        ...
        {gathering.games.length > 0 && (
          <div className="animate-fade-in-up animation-delay-200 space-y-3">
            <p className="font-semibold text-[11px] tracking-[0.15em] text-primary uppercase">
              Games
            </p>
        ...
        <div className="animate-fade-in-up animation-delay-200 space-y-3">
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm text-muted-foreground">
              ...
              {gathering.maxPlayers ? `/${gathering.maxPlayers}` : ''} joined
            </span>
          </div>
          ...
          {participants.length > 0 && (
            <div className="space-y-2">
              {participants.map((p) => {
                ...
                const isPending =
                  outgoingRequests.some((r) => r.addresseeId === p.userId) ||
                  incomingRequests.some((r) => r.requesterId === p.userId)
