...
export async function up(db: Kysely<unknown>): Promise<void> {
  await sql`CREATE TYPE schedule_type AS ENUM ('once', 'weekly', 'biweekly', 'monthly')`.execute(
    db,
  )
  await sql`CREATE TYPE gathering_status AS ENUM ('active', 'closed')`.execute(
    db,
  )
  ...
  await db.schema
    .createIndex('idx_gathering_host_id')
    .on('gathering')
    .column('host_id')
    .execute()
  await db.schema
    .createIndex('idx_gathering_zip_code')
    .on('gathering')
    .column('zip_code')
    .execute()
  await db.schema
    .createIndex('idx_gathering_next_occurrence_at')
    .on('gathering')
    .column('next_occurrence_at')
    .execute()
  await db.schema
    .createIndex('idx_gathering_status')
    .on('gathering')
    .column('status')
    .execute()
}
